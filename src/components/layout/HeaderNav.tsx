...
import { Sparkles, Sun, Moon, ChevronDown, Check, UserCheck, ShieldCheck, GraduationCap, Users, LogOut, BookOpen, Settings, Plus, PhoneCall, Code2, Globe, PlayCircle } from 'lucide-react';
...
export const accounts: UserAccount[] = [
  ...
  },
  {
    ...
    avatar: '/images/avatar_teacher1.jpg',
    ...
    <>
      ...
      <header className="navbar" style={{ position: 'sticky', top: 0, zIndex: 100 }}>
        <div className="navbar__inner container">
          ...
          {/* Right User Profile Dropdown Button */}
      ...
      {/* ─── MOBILE BOTTOM TAB BAR (Fixed at bottom for mobile screens <767px) ─── */}
      <nav className="nav-mobile-bottom">
        ...
        {currentUser.role === 'teacher' && (
          <>
            <button className={`mobile-tab-btn ${currentPage === 'teacher-dashboard' ? 'active' : ''}`} onClick={() => onNavigate('teacher-dashboard')}>
              <Users size={18} />
              <span>Кабинет</span>
            </button>
            <button className={`mobile-tab-btn ${currentPage === 'catalog' ? 'active' : ''}`} onClick={() => onNavigate('catalog')}>
        ...
        {currentUser.role === 'admin' && (
          <>
            <button className={`mobile-tab-btn ${currentPage === 'admin-dashboard' ? 'active' : ''}`} onClick={() => onNavigate('admin-dashboard')}>
              <ShieldCheck size={18} />
